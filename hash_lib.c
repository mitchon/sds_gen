...
unsigned char *LPS(unsigned char *array)
{
    ...
    unsigned long int longint_result [16] = {0};
    
    for (int j=0; j<16; j++)
    {
        for(int i=0; i<4; i++)
        {
            longint_result[j]=longint_result[j] << 8;
            longint_result[j]+=result2[i+j*4];
        }
    }
    ...
    for (int j=0; j<8; j++)
    {
        ...
        for (int i=0; i<32; i++)
        {
            b[i+32]=(longint_result[j*2+1] >> (31-i)) & 1;
            b[i]=(longint_result[j*2] >> (31-i)) & 1;
        }
...
unsigned char *hash512(unsigned char *msg, long len)
{
    ...
    while (len > 64)
    {
        ...
        unsigned char tmp_512[64] = {0};
        for (int i=0; i<3; i++)
            tmp_512[63-i]=(512 >> (8*i))& 0xFF;
    ...
    unsigned char *tmp_len = (unsigned char*)malloc(64);
    for (int i=0; i<64; i++)
        tmp_len[i]=0;
    for (int i=0; i<4; i++)
        tmp_len[63-i]=((len*8) >> (8*i))& 0xFF;
